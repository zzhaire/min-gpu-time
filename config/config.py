...
class SimulatorConfig:
    ...
    sharing_penalty_map: Dict[int, float] = field(
        default_factory=lambda: {
            1: 1.0,  # 独占: 100% 效率
            2: 0.85,  # 2任务: 85% 效率 (轻微干扰)
            3: 0.65,  # 3任务: 65% 效率 (显著干扰)
            4: 0.40,  # 4任务: 40% 效率 (严重干扰，开始不划算)
            5: 0.20,  # 5任务: 20% 效率 (拥堵)
        }
    )
    sharing_penalty_floor: float = 0.1  # 效率下限
...
class SchedulerConfig:
    ...
    min_gpu_time: MinGPUSchedulerConfig = field(default_factory=MinGPUSchedulerConfig)
    pollux: PolluxSchedulerConfig = field(default_factory=PolluxSchedulerConfig)
