...
def get_scheduler(scheduler_name: str, cluster: Cluster, scheduler_config=None):
    ...
    if scheduler_name == "min-gpu-time":
        cfg = (
            getattr(scheduler_config, "min_gpu_time", None)
            if scheduler_config
            else None
        )
        patience = cfg.patience_threshold if cfg else 1.1
        starvation_limit = cfg.starvation_limit if cfg else 2000.0
        return MinGPUTimeScheduler(
            cluster, patience_threshold=patience, starvation_limit=starvation_limit
        )
    ...
    if scheduler_name == "pollux-patient":
        cfg = getattr(scheduler_config, "pollux", None) if scheduler_config else None
        # 对于 Pollux Patient，为了极致优化 Min-GPU-Time，强制 alpha=0.0
        # 这样 Score = 1 / Cost，完全忽略 JCT (Speedup) 的影响
        alpha = 0.0
        patience = cfg.patience_threshold if cfg else 1.1
        starvation_limit = cfg.starvation_limit if cfg else 2000.0
        return PolluxPatientScheduler(
            cluster,
            alpha=alpha,
...
def summarize_results(output_dir: str):
    ...
    for res in results:
        print(
            f"{res['Scheduler']:<20} {res['Total Tasks']:<10} {res['Completed']:<10} "
...
def main():
    ...
    # 运行模式
    parser.add_argument("--run-all", action="store_true", help="运行所有调度算法并对比")

    # 绘图
    parser.add_argument("--plot", action="store_true", help="实验结束后绘制时空图")
    ...
    print("=" * 60)
    print(
        f"集群: {cluster_config.num_racks}机架 x {cluster_config.gpus_per_rack}GPU, "
        f"每GPU {cluster_config.gpu_memory}GB"
    )
    print(
        f"任务: {task_config.num_tasks}个, "
    ...
    if args.run_all:
        ...
        for scheduler_name in schedulers_to_test:
            ...
            metrics.save_to_tables(experiment_config.output_dir, scheduler_tag)

            # 绘图
            if plotter:
                tasks_file = os.path.join(
                    experiment_config.output_dir, f"tasks_{scheduler_tag}.csv"
                )
                plotter.plot_gantt_chart(
                    tasks_file, f"gantt_{scheduler_tag}.png", scheduler_name
                )
        ...
        if plotter:
            ...
            plotter.plot_gantt_chart(
                tasks_file, f"gantt_{scheduler_tag}.png", args.scheduler
            )
