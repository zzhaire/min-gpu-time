...
class Simulator:
    ...
    def run(self, tasks: List[Task], max_time: float = None):
        ...
        while self.current_time < max_time:
            ...
            # 调度新任务
            allocations = self.scheduler.schedule(pending_tasks, self.current_time)
            
            # 启动新分配的任务
            for task_id, gpu_ids in allocations.items():
                task = next((t for t in pending_tasks if t.task_id == task_id), None)
                if task:
            ...
            for task in running_tasks:
                ...
                if task.start_time is not None:
                    ...
                    if task.allocated_gpus:
                        penalty = self.cluster.calculate_penalty(task.allocated_gpus)
                        adjusted_duration = task.estimated_duration * penalty
